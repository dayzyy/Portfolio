...
export default function ToggleScroll() {
	...
	const toggle = (_) => {
		setToggled((prev) => {
			...
			document.querySelector("main").style.scrollSnapType = prev
				? "none"
				: "y mandatory";
			return !prev;
		});
	...
	useEffect(
		(_) => {
			...
			box.style.setProperty(
				"--pseudo-before-shadow",
				toggled
					? "0 -4px 5px var(--color-glow-on)"
					: "0 -4px 5px var(--color-glow-off)",
			);
		},
		[toggled],
	);
	...
	useEffect((_) => {
		//disable scrolling for first 3400ms after page loads, to wait for the start animations to finish
	...
	return (
		<div
			onClick={toggle}
			className="self-end toggle-scroll flex items-end gap-4 cursor-pointer"
		>
