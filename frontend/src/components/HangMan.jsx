...
export default function HangMan() {
    const words = [
	{ word: "pencil", description: "Used for writing or drawing, usually with an eraser." },
	{ word: "bottle", description: "A container for holding liquids." },
	{ word: "jacket", description: "Clothing you wear to keep warm." },
	{ word: "window", description: "A glass opening in a wall to let in light." },
	{ word: "basket", description: "Used to carry or store things, often woven." },
	...
	{ word: "pillow", description: "You rest your head on this when sleeping." },
	{ word: "cookie", description: "A sweet baked snack." },
	{ word: "banana", description: "A yellow fruit you peel." },
	{ word: "ticket", description: "You need this to get into a movie or bus." },
	...
	const all_guessed = [...word_letters].every(letter => guessed_set.has(letter))
	...
	setGameOver('loss')
	const timeout = setTimeout(_ => guessed_letters.current = Array.from(word.word), 500)
	...
	if (Array.from(word.word).includes(lastGuess)) {
	    ...
	    setUnGuessesLetters(prev => prev.filter(letter => letter != lastGuess))

	    clearTimeout(correctGuessTimer.current)
	    setCorrectGuess(true)
	    correctGuessTimer.current = setTimeout(_ => setCorrectGuess(false), 300)
	} else {
	...
	available_letters.current = available_letters.current.filter(letter => letter != lastGuess)
    ...
    return (
	<div id="hangman" className="mini-program flex-grow flex flex-col justify-around items-center">
	    <div className={`w-full h-[5rem] flex justify-around items-center transition-opacity duration-200 ${gameOver ? 'opacity-0' : ''}`}>
		<div className={`w-[3rem] h-[3rem]`}>
		</div>

		{!showResult &&
		    <div className={`w-[5rem] h-[5rem] grid place-items-center transition-shadow duration-500 ${wrongGuess ? 'wrong-guess' : ''}`}>
			{lastGuess && 
			    <h1 className={`last-guess transition-opacity duration-500 ${wrongGuess ? 'opacity-100' : 'opacity-0'}`}>{lastGuess}</h1>
			}
		    </div>
		}

		{showResult &&
		    <h1 className={`scale-0 transition-transform duration-300 ${growResult ? 'scale-100' : ''}`}>{gameOver == 'win' ? 'You have won!' : 'You have lost!' }</h1>
		}
